...
class JobRunner:
    ...
    def submit_arcpy_code(self, request: ArcPyCodeRequest) -> Dict[str, Any]:
        job_id = uuid4().hex
        output_dir = job_output_dir(self.output_root, job_id, request.project_name)
        log_path = output_dir / "job.log"
        config = model_to_dict(request)
        ...
        self.executor.submit(self._run_arcpy_code, job_id, request, output_dir, log_path)
        return job
    ...
    def _run_arcpy_code(
        ...
        self.store.update_job(job_id, status="running")
        try:
            result = self.code_runner.run_code(
                job_id=job_id,
                request=request,
                output_dir=output_dir,
                log_path=log_path,
            ...
            logger.exception("ArcPy code job %s failed", job_id)
            output_dir.mkdir(parents=True, exist_ok=True)
            runner_traceback = "RUNNER TRACEBACK:\n" + traceback.format_exc()
            if log_path.exists() and log_path.stat().st_size > 0:
                with log_path.open("a", encoding="utf-8") as log_file:
