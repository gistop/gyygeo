...
$Script:ProjectEnvName = "gyygeo-py3"
...
function Assert-ProjectPython {
    ...
    Assert-PathExists `
        -Path $PythonExe `
        -Message "gyygeo project Python was not found: $PythonExe. Run scripts\create-env.ps1 first."
    ...
    if ($resolvedProject -eq $resolvedBase) {
        throw "Refusing to use the original ArcGIS Pro Python environment. Use the cloned gyygeo-py3 environment instead."
    }
}
